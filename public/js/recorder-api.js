...
(function ($) {
  ...
  $.fn.AvRecorderMoz = function (id, conf) {
    ...
    var $reviewButton = $element.find('.av-recorder-review');
    var $nextButton = $element.find('.av-recorder-next');
    ...
    if(settings.constraints.video == true){
    	setStatus('Click \'Agree\' to enable your mic and camera.');
    }else{
    	setStatus('Click \'Agree\' to enable your mic.');
    }
...
//          recorder = new MediaRecorder(localStream, options);
          ...
          $element.trigger('refreshData', blob);
          $element.trigger('uploadFinished', blob);
	  			// sendBlob(blob);
          };
...
//        setStatus('Record with audio or video?');
    ...
    function sendBlob(blob, question){
    	//send audioBlob to server
        var formData = new FormData();
        formData.append("mediaBlob", blob[0], "blob");
        formData.append("action", 'recording_upload');
        formData.append("question", question);
        formData.append("anonymous", $('input[name="anonymous"]:checked').val());
        formData.append("security", settings.server_upload_endpoint.security);
        ...
        request.open('POST', settings.server_upload_endpoint.ajax_url, true);
...
//            $video.show();            
    ...
    function initializeButtons() {
      ...
      $nextButton.bind('click', function (event) {
        ...
        let questions_array = $(this).data('questions');
        let prev_question = +$(this).attr('current-question');
        sendBlob(blobs, questions_array[prev_question]);
        let next_question = prev_question + 1;
        if(questions_array.length == next_question){
          $element.find('.av-recorder').html('<p>Thank you for submiting your detailed review!</p>');
        }else{
          $('.qts-question').html('<b>Question ('+(next_question+1)+' of '+settings.server_upload_endpoint.selectable_questions+'):</b> '+questions_array[next_question]);
          $('button.av-recorder-next').attr('current-question', next_question);
          start();
        }
      });
...
//          $inputFid.val(data.fid);
          $recordButton[0].disabled = false;
          $recordButton[0].innerHTML = 'Record Again';
          playbackPreview();
          setStatus(settings.server_upload_endpoint.review_message);
        });
